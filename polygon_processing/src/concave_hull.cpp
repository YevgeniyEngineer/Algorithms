// Original author of the code: Alasdair Craig
// Code was published in https://www.codeproject.com/Articles/1201438/The-Concave-Hull-of-a-Set-of-Points
...
// Compile using g++ concave_hull.cpp -fopenmp -llz4
...
auto Usage() -> void
{
    std::cout << "Usage: concave.exe filename [-out arg] [-k arg] [-field_for_x arg] [-field_for_y arg] [-no_out] "
                 "[-no_iterate]\n";
    std::cout << "\n";
...
auto RemoveDuplicates(PointVector &points) -> void
{
    ...
    auto newEnd = unique(begin(points), end(points), [](const Point &a, const Point &b) { return PointsEqual(a, b); });
...
{
    auto itr = std::lower_bound(begin(list), end(list), p, [](const Point &a, const Point &b) { return a.id < b.id; });
...
auto SortByAngle(PointValueVector &values, const Point &from, double prevAngle) -> PointVector
{
    for_each(begin(values), end(values),
             [from, prevAngle](PointValue &to) { to.angle = NormaliseAngle(Angle(from, to.point) - prevAngle); });

    sort(begin(values), end(values),
         [](const PointValue &a, const PointValue &b) { return GreaterThan(a.angle, b.angle); });

    PointVector angled(values.size());

    transform(begin(values), end(values), begin(angled), [](const PointValue &pv) { return pv.point; });
...
auto RemoveHull(PointVector &points, const PointVector &hull) -> PointVector::iterator
{
    ...
    transform(begin(hull), end(hull), begin(ids), [](const Point &p) { return p.id; });

    sort(begin(ids), end(ids));

    return remove_if(begin(points), end(points),
                     [&ids](const Point &p) { return binary_search(begin(ids), end(ids), p.id); });
}
...
auto MultiplePointInPolygon(PointVector::iterator begin, PointVector::iterator end, const PointVector &hull) -> bool
{
    auto test = [&hull](const Point &p) { return !PointInPolygon(p, hull); };
...
auto TestAngle() -> void
{
    auto ToDegrees = [](double radians) { return radians * 180.0 / M_PI; };
